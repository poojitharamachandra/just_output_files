...
if __name__=="__main__":
    ...
    try:
      ...
      while(not (use_done_training_api and M.done_training)):
        ...
        print("default time budget :"+str(default_time_budget))
        print("remaining time : "+str(remaining_time_budget))
        # Train the model
    ...
    os.system("cp -v -R {} {}".format(os.path.join(output_dir, '*'), score_dir))
    logger.debug("Copied all ingestion output to scoring output directory.")
    #print("saving learned model... to ......."+output_dir+"/mymodel.pth")
    #print(M.pytorchmodel)
    #print(M.pytorchmodel.state_dict().keys())
    #torch.save(M.pytorchmodel,os.path.join(output_dir,"mymodel.pth"))
    #torch.save({'state_dict': M.pytorchmodel.state_dict()}, os.path.join(output_dir,'tmp.pt'))
